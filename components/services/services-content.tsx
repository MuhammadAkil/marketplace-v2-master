...
interface Service {
    id: string
    title: string
    status: "Not Yet Published" | "Publishing" | "Hidden" | "Draft"
    price: number
    orders: number
    isActive?: boolean
}
            ...
            <div className="space-y-3">
                {servicesState.map((service) => (
                    <div
                        ...
                        className={`rounded-lg pe-6 hover:bg-gray-100 transform transition-all duration-500 ${flashId === service.id ? "bg-blue-100" : "bg-white"}`}
                    >
                        <div className="flex items-center justify-between space-x-3 mb-6">
                            {/* Service Image */}
                            <div className="flex-shrink-0">
                                <Image width={148} height={83} src="/images/my-service.jpg" alt="Preview" className="w-[148px] h-[83px] rounded-xl object-cover" />
                            </div>
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                {isChangeMode && (
                                    <div className="flex items-center justify-end gap-2 min-w-[142px] h-6 min-h-6">
                                        {/* Show switch and label only if NOT "Not Yet Published" */}
                                        {service.status !== "Not Yet Published" ? (
                                            <>
                                                <Switch.Root
                                                    checked={service.isActive}
                                                    onCheckedChange={(checked) => handleToggleActive(service.id, checked)}
                                                    className={`w-[32px] h-[20px] rounded-full outline-none cursor-pointer transition-colors duration-200 ${service.isActive ? "bg-black" : "bg-gray-400 opacity-50"}`}
                                                    disabled={false}
                                                >
                                            ...
                                            // Preserve spacing even when switch is hidden
